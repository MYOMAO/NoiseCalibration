...
int RUDecodeData::decodeROF(const Mapping& mp)
{
  ...
  for (int icab = 0; icab < nCables; icab++) { // cableData is ordered in such a way to have chipIDs in increasing order
    ...
    int nhits = 0;
    while ((nhits = AlpideCoder::decodeChip(*chipData, cableData[icab], DOCLEAN, NHitCut, TotalPixelCut, TotalCalibrated, mNoisyPixels))) { // we register only chips with hits or errors flags set
      if (nhits > 0) {
