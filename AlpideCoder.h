	...
	namespace itsmft
	{
		...
		class AlpideCoder
		{
			...
			public:
				...
				template <class T>
					static int decodeChip(ChipPixelData& chipData, T& buffer, int& DOCLEAN, int& NHitCut, int& TotalPixelCut,int& TotalCalibrated, std::vector<std::map<int,int>>& mNoisyPixels)
					{
				...
				static bool add_hit(ChipPixelData& chipData, short row, short col, int& DOCLEAN, int& NHitCut, int& TotalPixelCut, int& TotalCalibrated,  std::vector<std::map<int,int>>& mNoisyPixels  ) {
